...
const renderItem = (type, item) => {
  return type === "$" ? (
    <Typography variant="body2" component="p">{`${type}${Math.round(item).toLocaleString("en-US")}`}</Typography>
  ) : (
    <Typography variant="body2" component="p">{`${Math.round(item).toLocaleString("en-US")}${type}`}</Typography>
  );
};
  ...
  return isStaked ? (
    <Box>
      ...
      <Box className="item" display="flex" justifyContent="space-between">
        ...
        <Typography component="p">{`${Math.round(100 - payload[0].value)}%`}</Typography>
      </Box>
    ...
    <Box>
      <Box className="item" display="flex" justifyContent="space-between">
        ...
        <Typography component="p">{`${Math.round(payload[0].value)}%`}</Typography>
      </Box>
      <Box className="item" display="flex" justifyContent="space-between">
        <Typography variant="body2" component="p">
          <span className="tooltip-bulletpoint" style={bulletpointColors[1]}></span>
